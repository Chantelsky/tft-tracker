...
router.get("/account/:region/:gameName/:tagLine", async (req, res) => {
  ...
  try {
    const data = await getAccountByRiotId(
      regionData.account,
      gameName,
      tagLine,
    );
...
router.get("/summary/:region/:gameName/:tagLine", async (req, res) => {
  ...
  try {
    ...
    const [rankedInfo, summonerInfo] = await Promise.all([
      getRankedInfo(regionData.platform, account.puuid),
      getSummonerInfo(regionData.platform, account.puuid),
    ]);
    ...
    res.json({ account, rankedInfo, summonerInfo, matches });
  } catch (error) {
